...
import pandas as pd
from main import WikiTopArticlesAnalyzer


class TestWikiParser(unittest.TestCase):
    @patch('main.WikiTopArticlesAnalyzer.requests.get')
    def test_get_top_wiki_articles(self, mock_get):
        mock_get.return_value = MagicMock(status_code=200)
        mock_get.return_value.json.return_value = {
            "items": [{"articles": [{"article": "Python", "views": 1000}]}]
        }
        result = WikiTopArticlesAnalyzer.get_top_wiki_articles("en.wikipedia", "2024", "01", "01")
    ...
    def test_transform_data(self):
        ...
        result = WikiTopArticlesAnalyzer.transform_data(df)
    ...
    def test_calculate_stats(self):
        data = {"article": ["Python", "Java"], "views": [1000, 1500], "date": ["2024-01-01", "2024-01-01"]}
        df = pd.DataFrame(data)
        mean_views, max_views, unique_articles = WikiTopArticlesAnalyzer.calculate_stats(df)
